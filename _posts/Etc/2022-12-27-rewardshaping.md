...
1. Feature Vector, $$f(s,a)$$는 $$n \times |A|$$ different fuction의 벡터이다.   
2. $$n$$은 state feature의 수이고, $$|A|$$은 action의 수이다. 각각의 함수는 state-action pair(s,a)의 값(value)을 추출한다.  
3. 함수 $$f_i(s,a)$$는 state-action pair (s,a)에서 i번째 feature를 추출한다.    
4. weight vector $$w$$ of size $$n \times |A|$$, 각각의 feature-action 쌍에 대해 하나의 weight이다.  
