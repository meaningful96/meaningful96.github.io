...
StAR의 모델 아키텍쳐를 보기 전 Translation-based 모델을 어느정도 숙지해야 한다. Translation-based 모델은 앞서 말했듯, <head, relation>의 임베딩을 <span style = "color:gold">**Translation fucntion**</span>을 이용해 신뢰성을 판단하는 모델이다. 여기서 중요한 것은 <u>Translation function의 출력값과 tail 임베딩 tail사이의 공간적인 관계성(Spatial relationship)을 통해 <span style ="color:gold"><b>구조 정보(Structural Information)</b></span>를 학습</u>한다는 것이다. 
