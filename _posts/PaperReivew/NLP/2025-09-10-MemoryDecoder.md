...
<center>p_{\text{kNN}}(y_t \vert x) \propto \displaystyle\sum_{(k_i, v_i) \in N(k_t, k)} \mathbf{1}_{y_t = v_i} \exp \left( - \frac{d(k_t, k_i)}{\tau} \right) </center>

이후 입력 쿼리로부터 LLM이 다음 토큰을 예측할 때, 동일한 레이어의 임베딩을 추출하여 데이터스토어에서 kNN 검색을 수행하고, 유사한 문맥들의 임베딩을 기반으로 다음 토큰 확률 분포를 계산한다. 

- **Next Token Prediction**
<center>p(y_t \vert x) = \lambda p_{\text{kNN}}(y_t \vert x) + (1 - \lambda)p_{\text{LLM}} (y_t \vert x)</center>
