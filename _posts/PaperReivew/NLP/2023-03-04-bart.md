...
BART는 <span style="color:#9e0000">**손상된 text를 입력받아 bidirectional 모델로 인코딩하고, 정답 text에 대한 likelihood를 autoregressive 디코더로 계산**</span>한다. 이런 설정은 noising이 자유롭다는 장점이 있다. 이 논문에서는 문장 순서를 바꾸거나 임의 길이의 토큰을 하나의 mask로 바꾸는 등의 여러 noising 기법을 평가한다. 
...
- **Token Deletion**: 랜덤 토큰을 삭제하고 이를 복구하는 방식이다. Masking과의 차이점은 <span style="color:#9e0000">**어떤 위치의 토큰이 지워졌는지 알 수 없다**</span>는 점이다.
...
- Pre-trained 모델의 성능은 task가 큰 영향을 미친다. 예를 들어 language model의 경우 generation task인 ELI5에서는 가장 좋았으나 classification task인 SQuAD에서는 가장 나빴다.
