...
논문에서는 **Knowledge-Injected Curriculum Pretraining(KICP)** 프레임워크를 소개한다. KICP는 사전 학습된 언어 모델을 위한 종합적인 지식 학습과 복잡한 추론 능력을 목표로 하는 일반적인 프레임워크로, <span style="color:gold">**지식 주입(KI)**</span>, <span style="color:gold">**지식 적응(KA)**</span>, <span style="color:gold">**커리큘럼 추론(CR)**</span>의 세 가지 주요 구성 요소로 이루어져 있다. KI는 지식 그래프(KG)에서 지식을 추출하여 문장으로 변환하고, KA는 원래의 언어 모델을 고정시켜 자연어 이해 능력을 유지하면서 생성된 코퍼스에서 지식을 학습한다. 마지막으로, CR은 인간의 추론 패턴을 따라 여러 난이도의 추론이 요구되는 코퍼스를 구성하고, 쉬운 것에서 어려운 것 순으로 언어 모델을 훈련시켜 모델 학습을 촉진한다 .
