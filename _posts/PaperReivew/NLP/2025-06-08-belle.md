...
첫 번째 모듈인 **Question Type Classifier**는 주어진 Multi-hop 질문을 Inference, Comparison, Temporal, Null의 네 가지 유형으로 분류한다. 이 과정은 LLM 기반의 In-Context Learning을 활용하여 수행되며, 입력 질문과 함께 여러 개의 QA 예제를 포함한 템플릿을 입력으로 받아, <span style="color:gold">**질문 유형을 예측**</span>하는 방식으로 동작한다. 해당 모듈은 향후 실행될 operator 조합을 결정하는 데 중요한 사전 지식으로 작용한다.
