...
# Problem Statement
**Draft & Verify**는 별도의 draft 모델 없이 <span style="color:red">**하나의 LLM으로 초안(draft) 생성과 검증(verify)을 모두 수행**</span>하는 ***self-speculative decoding***을 제안한다. 기존 speculative decoding은 몇 가지 문제점이 존재한다.
...
Draft & Verify의 핵심은 <span style="color:red">**하나의 LLM을 가지고 드래프팅과 검증을 모두 하는 것**</span>이다. 드래프팅 단계에서는 중간 레이어의 일부를 건너뛰어 빠르게 K개 후보를 만든 뒤( $$ p(\cdot \mid z^*,\cdot)$$ ), 원본 LLM $$p(\cdot \mid\vec 0, \cdot)$$으로 한 번의 forward pass로 검증한다.
