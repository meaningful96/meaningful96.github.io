...
먼저  $$\ell$$l$$째 레이어의 어텐션 스코어와 어텐션 가중치 행렬을 각각 $$A_{h, \ell}^S, A_{h, \ell}^W$$로 두고, 모든 헤드에 대해 합산(sum)하여 레이어 수준의 행렬 $$A_\ell^S$$와 $$A_\ell^W$$를 정의한다. 이 때 **정보 흐름의 방향(information flow)**을 $$A_\ell^W(i,j)$$로 정의하고, 이는 $$i$$번째 토큰에서  $$j$$번째 토큰으로의 정보 흐름을 의미한다.
