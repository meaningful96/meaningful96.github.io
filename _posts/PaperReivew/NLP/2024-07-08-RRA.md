...
5. 식(5) $$J_t = -D_{v,t} \log D'_{v,t} = - \sum_{c=1}^{V} d_{v,c} \log d'_{v,c}$$: 교차 엔트로피 손실 $$J_t$$를 계산한다. 
6. 식(6) $$L_{CE} = \frac{1}{T} \sum_{t=1}^{T} J_t$$: Cross Entropy 손실 함수 $$L_{CE}$$를 계산하여 파라미터를 업데이트한다.
