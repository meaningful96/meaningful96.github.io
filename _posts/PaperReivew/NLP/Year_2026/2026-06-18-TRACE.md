...
## 3.2. KG Generator
**KG Generator**는 <span style="color:red">**검색된 문서를 question-independent KG triple들로 변환**</span>한다. ****입력은 각 문서 $$d_i \in D_q$$이고, 출력은 해당 문서에서 생성된 triple 집합 $$G_{q,d_i}$$이다. 각 Wikipedia 문서는 title과 text로 구성되며, TRACE는 title을 head entity로 두고 text 내부 phrase를 tail entity로 삼아 `⟨title, relation, tail entity⟩` 형태의 triple들을 생성한다. 
