...
질문 $$q$$와 정답 $$a$$가 주어졌을 때,  관계 경로는 $$z = \{r_1, r_2, \cdots, r_l\}$$이고 추론 경로는 $$w_z(e_q, e_a) = e_q \xrightarrow{r_1} e_1 \xrightarrow{r_2} \cdots \xrightarrow{r_l} e_a$$이다. 여기서 입력으로 들어온 질문에 대해 신뢰할 수 있는 질문만을 고려한다면 $$Q(z)$$의 식은 근사 가능하다. 논문에서는 $$\mathcal{Z}$$의 확률 분포를 uniform으로 가정하였다.

<p align="center">
<img width="800" alt="1" src="https://github.com/user-attachments/assets/13e15692-12f6-41db-ac2e-06e7a7aa7ff4">
</p>

결론적으로 이러한 근사 방식을 이용하면 planning 뫃모듈에서 정의되는 손실 함수 $$\mathcal{L_{plan}}$$ 또한 위의 식처럼 변형이 가능하다.
