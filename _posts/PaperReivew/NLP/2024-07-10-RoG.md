...
여기서 오른쪽 항의 첫 번 째 term인 기댓값에 대한 부분은 retrieval-reasoning 모듈로부터, KL dievergency term은 planning 모듈로부터 만들어지는 term이다. $$Q(z)$$는 KG에 존재하는 모든 관계 경로 중, 신뢰할 수 있는 관계 경로의 posterior를 의미한다. 
...
## Module 2. Retrieval-Reasoning Module
Retrieval-Reasoning 모듈은 <span style="color:gold">**만들어진 관계 경로를 이용해 최종적으로 추론 경로를 찾고, 정답을 도출**</span>하는 모듈이다.
...
또한 모든 최단 거리 $$\mathcal{Z^{*}}$$ 중 top K개의 유망한 샘플의 최단 거리 관계 경로 $$\mathcal{Z_K^{*}}$$만을 고려하면 최종적으로 retrieval-reasoning 모듈에서 최적화하는 손실 함수 $$\mathcal{L_{reason}}$$은 위와 같이 정의된다.
...
planning 모듈을 거치면서 관계 경로 $$z$$가 생성되었으므로, retrieval-reasoning 모듈에서 주어진 것은 **질문**과 **관계 경로**이다. 주어진 질문과 관계 경로를 통해 추론 경로를 찾아내고, 이는 수식으로 다음과 같이 표현된다. 참고로 LLM에서 검색을 통해 만들어지는 가능한 모든 추론 경로가 추론을 하는데 사용된다. 검색을 위한 알고리즘은 다음과 같다.

<p align="center">
<img width="800" alt="1" src="https://github.com/user-attachments/assets/bc6f133a-d4ff-44b6-82c6-4e213770dda1">
</p>

최종적으로 retrieval-reasoning 모듈은 손실 함수  $$\mathcal{L_{reason}}$$를 최대화하는 방향으로 최적화를 하게 된다.
