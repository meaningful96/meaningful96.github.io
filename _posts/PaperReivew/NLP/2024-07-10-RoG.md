...
이 때, $$Q(z)$$는 KG에 존재하는 모든 관계 경로 중, 신뢰할 수 있는 관계 경로의 posterior를 의미한다. 질문 $$q$$와 정답 $$a$$가 주어졌을 때,  관계 경로는 $$z = \{r_1, r_2, \cdots, r_l\}$$이고 추론 경로는 $$w_z(e_q, e_a) = e_q \xrightarrow{r_1} e_1 \xrightarrow{r_2} \cdots \xrightarrow{r_l} e_a$$이다. 여기서 입력으로 들어온 질문에 대해 신뢰할 수 있는 질문만을 고려한다면 $$Q(z)$$의 식은 근사 가능하다. 논문에서는 $$\mathcal{Z}$$의 확률 분포를 uniform으로 가정하였다.
