...
**첫 번째 홉**에서는 질문 $$Q$$와 후보 페세지 $$p_i$$를 결합한다. 즉, 하나의 질문과 모든 패세지들과의 쌍(pair)을 각각 모델에 입력시켜 임베딩한다. 만약 질문 $$Q$$에 대해 10개의 Document가 있으면, 총 10개의 쌍을 임베딩하는 것이다. 이를 수식으로 나타내면 다음과 같다.
