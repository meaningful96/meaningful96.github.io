...
- **<span style = "font-size:120%">1) GNN 모델들에 "Attention Mechanism"이 적용된 적이 없다.</span>**
...
- **<span style = "font-size:120%">2) 전통적인 GNN은 Heterogeneous Graph를 처리하는데 부적합하다.</span>**
...
동그라미를 Movie, 세모를 Actor, 네모를 Director라고 했을 때, Move-Actor-Movie와 Movie-Director-Movie의 두 Meta-Path에 대한 Subgraph를 만드는 것이다. 이 Subgraph끼리 Attention을 하여 Attention Energy(Score)를 구한다.  이 때 구해진 energy(score)를 논문에서는 **Importance** 라고 지칭하였다.
