...
먼저 간단하게 Notation을 살펴보면, 저자는 head, relation, tail을 각각 subject, predictate, object라고 표현을 했다.
- Triple: \<head, relation, tail\> = \<source, predicate, object> 

Simple Context-Independent Transformer는 Bottom block의 맨 앞단 Transformer 하나 만을 이용해서도 Link prediction을 할 수 있다. 이 Transformer의 입력은 [CLS]토큰의 임베딩과 source entity의 임베딩, predicate의 임베딩이 된다. 
