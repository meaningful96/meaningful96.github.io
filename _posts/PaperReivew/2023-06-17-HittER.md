...
\[CLS\] 토큰 임베딩과 $$e_{src}$$, $$r_p$$를 각각 랜덤하게 초기화 한 후 세 개의 임베딩을 BERT와 마찬가지로 Transformer Encoder의 입력으로 집어넣는다. 그러면 Triple이 **Plausible**한지 아닌지를 판단해주는 결과값인 $$M_{e_{src}}$$가 출력된다. 같은 방식으로 이웃들 중 모든 후보 엔티티에 대하여 plausible score인 $$M_{e_{src}}$$를 구한 후 softmax 함수를 이용해 nomalization한다. 학습과정을 정리하자면 다음과 같다.
