...
논문에서는 이러한 방식은 너무 단순하기에 다른 sampling방식을 제안한다. 바로 tail 엔티티를 negative sampling 하되, **context를 고려해서 추출**하자는 것이다.(일종의 Context Sub-Graph) 정확하게 말하면, <span style = "color:gold">**Structural Context information**</span>을 사용하여 negative tail을 뽑는 것이다.
