...
논문에서는 추가적으로 Global information을 보존하기 <span style = "color:gold">**global node**</span>를 추가한다. global node는 자연어 처리의 pre-training 모델에서 **[CLS] 토큰과 유사한 역할을 수행**한다. 이 global node를 기존의 contextualized subgraph와 <span style= "color:green">학습가능한 가상의 거리(virtual distance) 또는 고정된 거리를 통하여 연결</span>한다.
