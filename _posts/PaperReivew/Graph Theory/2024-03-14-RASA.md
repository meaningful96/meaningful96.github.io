...
그리고 i)$$\mathcal{H}_{\text{KG}}$$의 각 릴레이션 토큰에 대해 해당하는 head와 tail 엔티티를 취하여 cosine 유사도를 사용해 LM 엔티티와의 **코사인 유사도**를 각각 계산한다. ii)이후 MP 토큰의 head 혹은 tail 엔티티는 가장 높은 유사도를 가진 LM토큰을 선택하여 $$\mathcal{H}_{text{LM}}$$에 매핑된다.
