...
Multiple-Choice Question-Answering (MCQA)는 question의 $$q$$의 단어 엔티티와 선택한 답변 $$a \in C$$ 엔티티를 Concatenation하여 시퀀스 $$X$$로 정의하고 이를 입력으로 넣는다. 그리고 사전 학습된 자연어 모델 $$\mathcal{g}_{\text{LM}}$$이 입력 시퀀스 $$X$$를 받아 출력된 값이 바로 Context Token $$\mathcal{H}_{\text{LM}}$$이 된다. 
