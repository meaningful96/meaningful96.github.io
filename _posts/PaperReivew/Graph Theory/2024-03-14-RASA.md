...
본 논문에서는 두 modality간의 feature를 합치기 전에 <span style="color:gold"><b>학습 가능한 임베딩</b></span> $$e_{\text{LM}}, e_{\text{KG}}$$을 정의해 각각의 modality에 더해주어 concatenation을 하는 방법을 제안한다. 이 두 임베딩은 query와 key token을 feature vector로써 modality source로 집어넣기 위함이다. 다시 말해, <span style="color:gold"><u>두 개의 서로 다른 modality 임베딩을 합쳐 하나의 새로운 feature vector를 만드는 것이기 때문에, 이 둘이 잘 합쳐지도록 align시켜 줄 학습 가능한 임베딩을 추가해준 것</u></span>이다. 최종적으로 attention의 결과는 $$\widetilde{\mathcal{Z}}$$로 나타내며, 이는 일반적인 scaled-dot attention의 결과와 동일하다.
