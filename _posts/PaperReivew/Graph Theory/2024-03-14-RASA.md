...
<p align="center">
  ...
  <center><figcaption>A node type among {$Z, Q, A, O$} is selected based on the node's source. To elaborate, $Z$ is a node which is deliberately inserted to connect all question and answer entities for context aggregation (similar to the cls token in Transformers), $Q$ and $A$ each denote the entities from the question and answer choice, respectively. $O$ is the “other” type of node that does not fall in the three categories, but constitutes the extracted subgraph.</figcaption></center>
</p>
