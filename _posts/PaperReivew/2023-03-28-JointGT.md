...
먼저 Notation을 살펴보면 Knowlege Graph $$\mathcal{G} = (\mathcal{V,E})$$ 이다. $$\mathcal{V} = {e_1, e_2, \cdots, e_{|\mathcal{V}|}}$$로 엔티티(Entity) 집합을 의미하고, $$\mathcal{E} = (r_{ij})_{|\mathcal{V}| \times |\mathcal{V}_{ }|}$$로 엔티티와 연결된 릴레이션(relation)을 의미한다.
