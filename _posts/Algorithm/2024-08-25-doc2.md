...
**시간 복잡도**에서 재귀 알고리즘의 복잡도 $$O(T)$$는 <span style="color:red">**재귀 호출 횟수 $$R$$**와 각 호출에서의 계산 시간 복잡도 **$$O(s)$$**의 곱</span>으로 표현된다. 예를 들어, 2번 문제의 문자열을 역순으로 출력하는 재귀 함수의 경우, 문자열의 길이 $$n$$만큼 재귀적으로 호출되며, 각 호출에서 한 번의 문자 접근이 이루어지므로 시간 복잡도는 $$O(n)$$이다.
