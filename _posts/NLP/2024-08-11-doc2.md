...
**Candidate 1**:
- 일치하는 bigram: "The Opportunity", "Opportunity rover", "rover is", "is on", "on Mars"
- **Precision** = $$\frac{5}{7} = 0.714$$

**Candidate 2**:
- 일치하는 bigram: "A NASA", "NASA rover", "rover is", "is fighting", "fighting a", "a massive", "on Mars"
- **Precision** = $$\frac{7}{9} = 0.778$$

<span style="font-size:105%">**4-gram Pecision**</span>    
**Candidate 1**:
- 일치하는 4-gram: "The Opportunity rover is", "Opportunity rover is on", "rover is on Mars"
- **Precision** = $$\frac{3}{5} = 0.6$$

**Candidate 2**:
- 일치하는 4-gram: "NASA rover is fighting", "rover is fighting a", "is fighting a massive", "fighting a massive storm"
...
**ROUGE(Recall-Oriented Understudy for Gisting Evaluation)**는 주로 <span style="color:red">**텍스트 요약 시스템의 성능을 평가**</span>하기 위해 사용되는 지표로, 기계 번역이나 텍스트 생성 시스템의 평가에도 널리 활용된다. ROUGE는 참조 텍스트와 생성된 텍스트 사이의 유사성을 측정하며, 다양한 변형 지표를 통해 평가를 진행한다. 특히 **ROUGE는 `Recall`을 중심으로 평가**를 하지만, `Precision`과 `F1-Score`도 함께 고려하는 다양한 지표를 제공한다.
