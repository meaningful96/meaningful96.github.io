...
GCN의 가장 기본적인 형태는 비대칭 정규화(Asymmetric Normalization)를 사용하여 각 노드의 이웃 정보를 집계한다.
...
<p align="center">
<img width="700" alt="1" src="https://github.com/user-attachments/assets/a2f9bec4-27f8-41ca-87fe-bf185f2263f0">
</p>
...
<center>$$H^{(k)} = \sigma \left( \tilde{D}^{-\frac{1}{2}} \tilde{A} \tilde{D}^{-\frac{1}{2}} H^{(k-1)} W_{k-1} \right)$$</center>

중요한 점은 GCN은 자기 정보를 취합하기 위해 Self-Loop를 추가했기 때문에, 상호작용이 없는 노드에 대해서는 임베딩 자체를 생성해내지 못한다. 즉, GCN은 <span style="color:red">**Transductive**</span>하다. 따라서 한 번도 보지 못한 새로운 노드가 등장하면 노드 임베딩과 마찬가지로 전체 그래프를 다시 학습해야 한다.
