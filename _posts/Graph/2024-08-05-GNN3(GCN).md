...
GCN은 여기서 필터 행렬 $$\mathbf{W}$$와 바이어스 $$\mathbf{b}$$를 학습한다. CNN에서 사용되는 합성곱 연산은 커널을 움직여가며 커널 크기에 해당하는 정보를 한 곳으로 모으는 과정을 의미한다. 그래프에서는 타겟 노드와 연결된 이웃 정보를 가중합 평균(Weighted Sum Average)하여 합성곱 효과를 만들어 낸다.
