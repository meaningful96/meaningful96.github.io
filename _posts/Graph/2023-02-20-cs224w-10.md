...
- Knowledge Graph Completion
    ...
    - Knowledge Graph Completion은 다시 Link prediction in KGs, Triple Classification, Relation Prediction으로 나눠진다.
        -  Link Prediction in KGs: ($$h,r, ?$$) , ($$?, r, t$$) 에서 ?를 찾아내는 문제이다.
        - Triple Classification: ($$h,r,t$$)의 참/거짓을 알아내는 문제이다. 
        - Relation Prediction: ($$h,?,t$$)가 주어졌을 때 Head와 Tail의 관계성을 나타내는 Relation Edge를 찾는 문제이다.
...
이 Connectivity Pattern이 중요한 이유는, 만약 가지고 있는 Data set이 Symmetric과 1-to-N relation을 만족할 경우 이 두 가지 패턴을 만족하는 모델을 사용해 문제를 해결하는 것이 가장 성능적으로 우수하기 때문이다.

따라서, 모델이 무슨 Pattern에 대해서 참인지도 파악해야 한다.
