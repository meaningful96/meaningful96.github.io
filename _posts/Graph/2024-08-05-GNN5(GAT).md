...
각 헤드의 출력을 결합(concatenate)하거나 평균(average)하여 최종 출력 벡터 $$\mathbf{h_1}^{'}$$ 을 만든다. 그림에서 보이는 "`concat/avg`"는 이 과정을 나타낸다. 최종적으로 $$\mathbf{h_1}^{'}$$가 $$v_1$$의 새로운 특징 벡터가 된다.

## GAT with Multi-head Attention 수식 정리
