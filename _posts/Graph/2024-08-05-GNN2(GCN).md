...
1. **이웃 정보를 취함**:
  - $$a_v^{(l)} = \text{AGG}^{(l)}\left(\left\{m_u^{(l-1)} \mid u \in N(v)\right\}\right)$$
...
3. **결합 과정**:  
  - $$ h_v^{(l)} = \text{Combine}^{(l)}\left(a_v^{(l)}, m_v^{(l-1)}\right)$$   
  - $$a_v^{(l)} = \text{AGG}^{(l)}\left(\left\{m_u^{(l)} \mid u \in N(v)\right\}\right)$$
  - $$m_u^{(l)} = W^{(l)} h_u^{(l-1)}$$
  - $$m_v^{(l-1)} = B^{(l)} h_v^{(l-1)}$$   
