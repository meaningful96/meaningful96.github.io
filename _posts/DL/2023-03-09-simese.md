...
정리하자면, Classification을 푸는데 기존의 레이블링된 데이터를 이용한다면, 방대한 양의 데이터를 필요로 하고, 데이터가 추가적으로 들어오면서 Class의 수가 늘어날 경우 모델을 새로 학습해야하는 불상사가 생긴다. 실제로는, Task에 따라 소량의 데이터를 가지고 학습해야하는 경우도 생기므로 이를 위한 해결책이 바로 **One-Shot Learning**이다.
