...
**S2) Pruning**
두 번째 과정은 **Pruning**이다. 이때 Pruning은 학습된 가중치의 값을 기준으로 수행된다. 앞선 단계에서 이를 위해 희소화를 도입했으며, Pruning 단계에서는 한 개의 가지를 제외하고 모두 제거한다. 위 그림을 보면 Pruning이 완료된 후 입력과 출력이 한 개의 라인으로 연결된 모습을 볼 수 있다. 즉, 입력값 x로부터 출력값 y가 나오는 과정이 곱하기와 더하기만으로 표현될 수 있는 상태가 된 것이다.
