...
반면, **KAN**은 <span style="color:#9e0000">**Kolmogorov-Arnold Representation Theorem**</span>를 기반으로 모델을 전개한다. KAN은 거창해 보이지만, 사실 '**spline**과 **MLP**의 조합에 불과하다'고 할 수 있다. 즉, spline 모델과 MLP를 결합한 것에 지나지 않는다.
...
이를 통해, 이 수식이 결론적으로 2-layer KAN에 대응된다는 것을 알 수 있다. 또한, 각각의 함수 $$\phi_{q, p}$$, $$\Phi$$들을 학습이 가능해야 하므로 [spline](https://meaningful96.github.io/deeplearning/spline/)을 이용하여, 각 함수들의 계수(coefficient)를 학습한다. KAN에서는 <span style="color:#9e0000">**B-spline**</span>을 이용한다.
