...
Receiver Operating Characteristic Curve (ROC Curve)는 "수신자 판단 곡선"이라는 뜻을 가진다. ROC Curve는 이진 분류 모델의 성능을 시각적으로 평가하는 지표로, 임계값을 변화시켰을 때의 Sensitivity나타낸다. **Sensitivity**란 위에서 언급하였듯이, Recall과 식이 동일하며, 의미를 이해하기 쉽도록 이를 <span style="color:#9e0000">**True Positive Rate(TPR)**</span>이라고 한다. 그러면 **1 - Specificity**는 자연스럽게 <span style="color:#9e0000">**False Positive Rate(FPR)**</span>가 된다. 
...
따라서 AUC는 ROC 커브를 요약하는 한 지표로, <span style="color:#9e0000">**AUC 값이 높을수록(즉, Positive와 Negative를 잘 구분하는)**</span> 성능이 좋은 모델이다. AUC값에 따른 case를 시각화하면 다음과 같다.
