...
value값에 따라 clipping하는 방법의 핵심은 <span style="color:#9e0000">**최소 클리핑 값(minimum cliping value)과 최대 클리핑 값(MAXIMUM clipping value)**</span>를 정하는 것이다. 만약에, gradient값이 threshold값(문턱값, 임계값)을 넘어가면 그 exploading gradient 값을 threshold로 클리핑 하는 것이다. 만약 gradient값이 minimum threshold보다 작으면 이 역시 클리핑해서 최소 임계값까지 올려놓는다.  
